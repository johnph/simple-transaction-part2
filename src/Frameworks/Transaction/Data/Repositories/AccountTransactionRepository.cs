namespace Transaction.Framework.Data.Repositories
{
    ...
    public class AccountTransactionRepository : IAccountTransactionRepository
    {
        ...
        public async Task<IEnumerable<AccountTransactionEntity>> Get(int accountNumber, DateTime startDate, DateTime endDate)
        {
            return await _accountTransactionEntity.Where(i => i.AccountNumber == accountNumber && i.Date >= startDate && i.Date <= endDate).ToListAsync();
        }
    }
}
