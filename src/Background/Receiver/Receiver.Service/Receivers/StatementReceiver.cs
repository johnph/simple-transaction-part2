namespace Receiver.Service.Receivers
{
    ...
    public class StatementReceiver : ScopedReceiver
    {
        ...
        public StatementReceiver(IBus bus, Func<string, IMessageProcessor> processor, IOptions<QueueNames> options, IServiceScopeFactory serviceScopeFactory, IRetryHelper retryHelper, ILogger<StatementReceiver> logger) : base(serviceScopeFactory, retryHelper, logger)
        {
            if (options == null)
            {
        ...
        {
            await _processor("statement").ProcessMessageAsync(qMessage);
        }

        public override async Task ProcessInScope(IServiceProvider serviceProvider)
        {
            await Task.Run(() => _bus.Receive<StatementMessage>(_queueNames.Statement, async (qMessage) => {
